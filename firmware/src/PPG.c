...
/* ══════════════════════════════════════════════════════════════════════════
 * APPLICATION LAYER
 * Timer-based PPG sampling at 200Hz — mirrors IMU timer approach.
 * ppg_init() starts the timer, no need to call ppg_read() from main.
 * ══════════════════════════════════════════════════════════════════════════ */
...
/* Timer callback (interrupt context) - just submits work */
void ppg_handler(struct k_timer *timer_id)
{
    k_work_submit(&ppg_work);
}

/* Work handler (work queue thread) - safe to do I2C here */
...
int ppg_init(void)
{
    ...
    /* Start timer at 200Hz = 5000us period */
    k_work_init(&ppg_work, ppg_work_handler);
    k_timer_start(&ppg_timer, K_USEC(5000), K_USEC(5000));

    printk("MAX30102 ready, sampling at 200Hz\n");
    return 0;
}